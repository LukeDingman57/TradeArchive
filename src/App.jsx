...
function RulesSettingsPanel() {
  ...
  const loadRules = async () => {
    ...
    if (error) {
      ...
      setRulesMessage(`Could not load rules: ${error.message}`);
    } else {
      setRules(data || []);
    }
  ...
  return (
    <div style={rulesSettingsStyles.card}>
      <div style={rulesSettingsStyles.headerRow}>
        <div>
          <div style={rulesSettingsStyles.title}>My Trading Rules</div>
          <div style={rulesSettingsStyles.subtext}>
            These rules show up inside Add Trade so you can track discipline and rule compliance.
          </div>
        </div>
      </div>

      <div style={rulesSettingsStyles.addRow}>
        <input
          value={newRule}
          ...
          style={rulesSettingsStyles.input}
        />
        <button
          type="button"
          onClick={addRule}
          disabled={savingRule}
          style={rulesSettingsStyles.addButton}
        >
          {savingRule ? "Adding..." : "+ Add Rule"}
        </button>
      </div>

      {rulesMessage ? <div style={rulesSettingsStyles.message}>{rulesMessage}</div> : null}

      <div style={rulesSettingsStyles.list}>
        {loadingRules ? (
          <div style={rulesSettingsStyles.empty}>Loading rules...</div>
        ) : rules.length ? (
          rules.map((rule) => (
            <div key={rule.id} style={rulesSettingsStyles.ruleRow}>
              <div style={rulesSettingsStyles.ruleName}>{rule.rule_name}</div>
              <button
                type="button"
                onClick={() => deleteRule(rule.id)}
                style={rulesSettingsStyles.deleteButton}
              >
                Delete
              </button>
            </div>
          ...
          <div style={rulesSettingsStyles.empty}>
            No custom rules yet. Add rules like “No news trades,” “Trade with HTF bias,” or “Max 2 trades per day.”
          </div>
        )}
      </div>
    </div>
